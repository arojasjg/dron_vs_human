...
- [~] CBT-M2 · Arquetipos de bot invisibles; bots dañados sin vida — `src/game.ts:655`. **BARRA HECHA (a3315d4):** el swarm se difundía 100/100 → la barra del avatar (ya existente, oculta a vida llena) nunca aparecía. Ahora difunde `frac` real (5º elem de la fila 'ai') → barra en bots dañados. `botHpFrac()` pura; compat `row[4]??1`. **PENDIENTE CBT-M2b (visual, sesión Chrome):** tinte/escala por arquetipo (tank≠chaser) — requiere difundir kindIdx + mapear tint; el look lo verifica el usuario. Ojo posible clutter de barras en refriega.
- [ ] CBT-M2c · _(nuevo, ciclo 29 — hallazgo del review)_ `buildScoreboard` (`roles.ts:148`) no filtra ids negativos → los bots (id neg) podrían salir como filas `🤖 0/0/0` en el scoreboard TAB. Pre-existente (no lo introdujo CBT-M2). Verificar si el caller le pasa bots; si sí, filtrar `id>0` como hace `setTeam` (hud.ts:409).
