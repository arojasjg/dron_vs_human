...
export class Game {
  ...
  private readonly aiBots = new Map<number, { x: number; y: number; z: number; frac: number; kind: AiKind }>(); // last-known bot positions (for shooting them)
  ...
  private aiFrame(dt: number): void {
    ...
    if (this.hosting && s && !this.matchOver) {
      ...
      this.aiBots.clear();
      for (const b of s.list) this.aiBots.set(b.id, { x: b.x, y: b.y, z: b.z, frac: botHpFrac(b.hp, b.maxHp), kind: b.kind });
      ...
      if (this.aiBcast <= 0 && this.net.connected) {
        this.aiBcast = 0.07;
        this.net.send({ t: "ai", b: s.list.map((b) => [b.id, +b.x.toFixed(2), +b.y.toFixed(2), +b.z.toFixed(2), +botHpFrac(b.hp, b.maxHp).toFixed(3), kindIdx(b.kind)]), k: this.sessionKills, w: s.wave });
      }
    }
    this.renderBots();
  }
  ...
  private renderBots(): void {
    for (const [id, p] of this.aiBots) this.remotes.upsert(-id, p.x, p.y, p.z, 0, 0, 0, 1, 100 * p.frac, "drone", 100, 0, 0, 0, 0, "", undefined, undefined, 0, 0, 0, archTint(p.kind));
  }
  ...
  private onNet(m: NetMsg): void {
    if (m.t === "hello") {
      ...
      if (this.mode === "coop" && !this.hosting) { // peers render the host's swarm from its broadcast
        this.aiBots.clear();
        for (const row of m.b as number[][]) this.aiBots.set(row[0], { x: row[1], y: row[2], z: row[3], frac: row[4] ?? 1, kind: kindFromIdx(row[5] ?? 0) });
